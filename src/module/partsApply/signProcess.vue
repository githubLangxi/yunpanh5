	...
	<div class="signprocess" v-loading="loading">
		<li v-for="list in signprocess.stateList">
			...
			<div class="statusText">
				...
				<span style="margin-top:6px;font-size:14px;" v-if="signprocess.currentStateValue>=list.stateValue?true:false">{{list.optTime}}</span>
			</div>
		</li>
	</div>
</template>
...
export default {
  ...
  methods: {
    loadData: function() {
      this.loading = true;
      var apiUrl = "api/PartsApply/PartsApplyProGress/" + this.partsapplyid;
      rt
        .get(apiUrl)
        .then(res => {
