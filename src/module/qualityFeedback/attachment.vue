<template>
  <rt-section id="area9" title="附件明细">
    <template slot="toolbar">
      ...
      <rt-button text="上传" type="add" @click="uploadFile" v-if="visible"/>
      <rt-button type="delete" @click="deleteData" v-if="visible"/>
    </template>
    <el-table ref="gridTable" :data="attachmentData" v-loading="loading">
      <el-table-column type="selection" width="35" v-if="visible">
      </el-table-column>
  ...
  methods: {
    ...
    fetchRecords: function() {
      if (this.qualityfeedbackid) {
        this.loading = true;
        var requestStr = '../api/WorkOrder/Annotation/' + this.qualityfeedbackid;
    ...
    onFileChange: function() {
      var _this=this;
      var id = _this.qualityfeedbackid;
      ...
      else
      {
        ...
        reader.onload = function(f) {
          ...
          if (file.size >= _this.fileAutoSize) {
            ...
            annotation["id"] = id;
            annotation["entityname"] = "new_srv_partsapply";
