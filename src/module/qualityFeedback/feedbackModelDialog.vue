    ...
    <template slot="toolbar">
      ...
      <rt-button type="delete" @click="deleteDetail" v-if="visible"/>
		...
		<el-table stripe ref="tableData" max-height="500" v-loading="loading" v-bind:data="tableData">
            ...
            <el-table-column prop="new_srv_userprofile_idName" label="条码">
            </el-table-column>
  			    <el-table-column prop="new_product_idName" label="机型">
            </el-table-column>
            <el-table-column prop="new_aux_serialidName" label="机器系列">
            </el-table-column>
            <el-table-column prop="new_productmodule_idName" label="产品类型">
            </el-table-column>
            <el-table-column prop="new_aux_producttypeidName" label="产品小类">
            </el-table-column>
	    </el-table>	
...
export default {
  ...
  methods: {
    _fetchRecords: function() {
      var apiUrl =
        "../api/QualityFeedback/GetFeedbackModel?id=" + this.qualityfeedbackid;
      rt
        .get(apiUrl)
        .then(res => {
          this.tableData = res;
    ...
    gotoAddPage() {
      //创建记录
      this.$router.push({ name: "feedbackModelCreate" , params: { id: this.qualityfeedbackid ,name: this.qualityfeedbackname} });
    },
    //明细删除
    deleteDetail: function() {
      ...
      this.$confirm(this.$t("ThisActionDeletePerpetually"), this.$t("Tip"), {
        ...
        .then(() => {
          ...
          var deleteStr = "../api/QualityFeedback/DeleteFeedbackModel";
          rt
