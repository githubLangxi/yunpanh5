...
export default {
  ...
  methods: {
    ...
    show: function(ev) {
      ...
      var lookupQueryStr = "../api/QualityFeedback/GetError";
      this.loading = true;
     
      if (this.lookupCondition) {
        params.filter += "new_name=" + this.lookupCondition + ",";
      }
      if (!rt.isNullOrWhiteSpace(this.errorgroupid)) {
        params.filter += "new_srv_errorgroup_id=" + this.errorgroupid+ ",";
      }
      if (!rt.isNullOrWhiteSpace(this.productgroupid)) {
        params.filter += "new_productgroup_id=" + this.productgroupid+ ",";
      }
