...
export default {
  data: function() {
    return {
      ...
      entityName: 'new_srv_stocksite',//实体名
      attributeList: 'new_name',//后台所需排序条件
      stationAttr: 'new_station_id',
      filterList: '',
      lookupOptions: [{ label: '单号', name: 'new_name' }],//lookup显示列
      dialogFormVisible: false,//lookup是否可见
      attrList: [],//暂存lookOptions列的数组
      formLabelWidth: '80px',//Label默认宽度
      //tableData: [],

      /*LookUp回写问题，无法识别editdata.new_stocksite_sendid,这里作为中间量，后期修改*/
  ...
  methods: {
    ...
    //工作单打开lookup
    workorderlookup() {
      this.handleclick('new_stocksite_sendid');
    },
    //lookup快速查询
    lookupsearch() {
      this.handleclick();
    ...
    lookupSelect() {
      ...
      if (this.currentRow == null || this.currentRow == '') {
        ...
        this.$emit('send-stocksiteid', [this.currentRow.new_id, this.currentRow.new_name])
      }
      this.handleclick('new_stocksite_sendid')
    },
    lookupCancel() {
