<template>
  <div>
    <rt-section id="area14" :title="$t('annotation')">
      <template slot="toolbar">
        <input type="file" value="请点击上传附件" @change="onFileChange" style="display:none" ref="upload" />
        <rt-button :text="$t('BUTTON_UPLOAD')" type="add" @click="uploadFile"  v-if="this.workOrder.canEdit"/>
        <rt-button type="delete" @click="deleteData" v-if="this.workOrder.canEdit" />
      </template>
  ...
  methods: {
    ...
    _fetchRecords: function () {
      if (this.workOrder && this.workOrder.id ) {
        this.loading = true;
        var requestStr = "../api/WorkOrder/Annotation/" + this.workOrder.id;
        rt
          .get(requestStr)
          .then(res => {
      ...
      if (ifSame) {
        ...
        reader.onload = function (f) {
          ...
          if (file.size >= that.fileAutoSize) {
            ...
            annotation["entityname"] = "new_srv_workorder";
