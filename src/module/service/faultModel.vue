<template>
    <rt-section id="area7" title="故障明细">
        <template slot="toolbar">
            <rt-button type="add" @click="addFaultModel" />
        ...
        <el-table ref="gridTable" :data="faultData" @row-dblclick="showFaultModel"  v-loading="loading">
            ...
            <el-table-column label="条码" prop="new_name">
            </el-table-column>
            <el-table-column label="故障现象" prop="new_srv_error_idName">
            </el-table-column>
            <el-table-column label="故障原因" prop="new_srv_errorgroup_idName">              
            </el-table-column>
            <el-table-column label="维修项目" prop="new_srv_workhours_idName" >
            </el-table-column>
            <el-table-column label="配件小类" prop="new_errormodule_idName" >
            </el-table-column>
            <el-table-column label="换件类型" prop="new_srv_errorassembly_idName" >
            </el-table-column>
        </el-table>
        ...
        <faultModelDialog ref="faultModelDialog" :work-order="this.workOrder"  @reLoad="reLoadfault"></faultModelDialog>
    </rt-section>
</template>

<script>
import faultModelDialog from "./faultModelDialog";
...
export default {
  ...
  methods: {
    ...
    deleteData() {
      ...
      this.$confirm("此操作将永久删除该记录,是否继续", "提示", {
        ...
        .then(() => {
          ...
          var requestStr = "../api/WorkOrder/DeleteFault";
          rt
    ...
    reLoadfault: function() {
      this._fetchRecords();
      this.$emit("reLoad");
    },
    showFaultModel: function(row) {
      this.$refs.faultModelDialog.show(row.id);
    },
    addFaultModel: function() {
      this.$refs.faultModelDialog.show("");
    },
