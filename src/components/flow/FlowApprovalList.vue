  ...
  <div>
    <div id="approveState">
      <div v-for="(state,index) in stateList" :key="state.Status" class="nodelist">
        <div class="node">
          <i class="node-icon" v-bind:class="{'icon-approving':(state.new_status !== 2 && state.new_status !== 0),'icon-approved':state.new_status === 2,'icon-submit':state.new_status === 0}"></i>
          <ul>
            <li class="owner">{{state.new_userName}}</li>
            <li class="date">{{state.new_date}}</li>
          </ul>
        </div>
    ...
    <div id="approveTrack">
      <ul>
        <li v-for="(track,index) in trackList" v-bind:class="{last: index === trackList.length - 1}" :key="track.Date">
          ...
          <div>
            <span class="date">{{track['new_date']}}</span>
            <span class="action">[{{track['new_actionName']}}]</span>
            <span class="step">{{track['new_stepName']}}</span>
            <span class="owner">{{track['new_userName']}}</span>
          </div>
          <div class="memo">
            <span>{{track.new_memo}}</span>
          </div>
        </li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    getStateList() {
      ...
      rt
        .get(
          "../api/CrmFlow/GetFlowApproveProgressList?entityName=" +
            this.entityname +
            "&entityId=" +
            this.id
        )
        .then(function(res) {
          if (res != null && res.length > 0) {
            ...
            // for(var i in res){
            // submit["OwneridName"] = res[i]["new_userName"];
            // submit["ApproveDate"] = res[i]["new_date"];
            // submit["Status"] = res[i]["new_status"];
            // }
    ...
    getTrackList() {
      ...
      rt
        .get(
          "../api/CrmFlow/GetFlowApproveLogList?entityName=" +
            this.entityname +
            "&entityId=" +
            this.id
        )
...
#approveTrack ul li .node-icon {
  ...
  width: 11px;
  height: 11px;
  background: url(//misc.360buyimg.com/user/order/0.0.1/css/i/order-icons.png) -21px -72px
    #fff no-repeat;
