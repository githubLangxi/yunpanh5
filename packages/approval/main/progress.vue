...
export default {
  ...
  methods: {
    ...
    getStateList() {
      let that = this;
      rt.get("../api/CrmFlow/GetFlowApprovalStep/" + this.id)
        .then(function (res) {
          if (res != null && res.length > 0) {
            ...
            submit["OwneridName"] = res[0]["ApplyUserName"] + " 提交";
            submit["ApproveDate"] = res[0]["ApplyDate"];
    ...
    getTrackList() {
      let that = this;
      rt.get("../api/CrmFlow/GetFlowApprovalTrack/" + this.id)
        .then(function (res) {
          that.trackList = res;
        }).catch((error) => {
          rt.showErrorToast(error);
